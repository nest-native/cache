/**
 * Audits the *published* supply-chain surface of `@stalefree/core`.
 *
 * The core package publishes `"dependencies": {}` (every third-party
 * integration — `drizzle-orm`, `pg`, and the database drivers — is an OPTIONAL
 * peer the consumer brings themselves, used only by the store/bus subpaths),
 * so the only third-party code a consumer actually installs for that empty
 * production closure is whatever npm pulls in for it. Auditing the monorepo
 * root instead would flag advisories that live exclusively in the
 * dev/peer/sample tree (e.g. transitive dependencies of the optional
 * `@nestjs/platform-express` peer used by sample apps) — none of which can
...
const npmCache = fs.mkdtempSync(path.join(os.tmpdir(), 'stalefree-core-audit-cache-'));
const consumerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'stalefree-core-audit-consumer-'));
...
try {
  ...
  npm(['run', 'build', '--workspace', '@stalefree/core'], { cwd: repoRoot, stdio: 'inherit' });

  const packOutput = npm(
    ['pack', '--json', '--workspace', '@stalefree/core', '--pack-destination', consumerDir],
    { cwd: repoRoot },
  );
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: 'stalefree-core-audit-consumer', version: '0.0.0', private: true }, null, 2)}\n`,
  );
