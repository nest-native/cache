...
const tempCache = fs.mkdtempSync(path.join(os.tmpdir(), 'stalefree-core-pack-'));

try {
  const rawOutput = execFileSync(
    npmExecutable,
    ['pack', '--dry-run', '--json', '--workspace', '@stalefree/core'],
    {
      cwd: repoRoot,
      encoding: 'utf8',
      env: {
  ...
  const requiredFiles = [
    ...
    'dist/index.js',
    'dist/index.d.ts',
    // Store/bus subpath entrypoints (./socket, ./postgres, ./sqlite, ./mysql)
    // are added here as they land in the package.json exports map.
  ];
